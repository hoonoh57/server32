...
Friend NotInheritable Class StockPoolDbSettings
    ...
    Public Shared Function Load() As StockPoolDbSettings
        ...
        For Each candidate In EnvironmentFileCandidates()
        ...
        Dim portValue As UInteger = 3306UI
        Dim rawPort As String = GetValue(values, "MYSQL_PORT", "3306")
        If Not UInteger.TryParse(rawPort, NumberStyles.None, CultureInfo.InvariantCulture, portValue) OrElse portValue = 0UI OrElse portValue > 65535UI Then
    ...
    Private Shared Function EnvironmentFileCandidates() As IEnumerable(Of String)
        ...
        If Not String.IsNullOrWhiteSpace(explicitFile) Then result.Add(explicitFile.Trim())

        result.Add(Path.Combine(Environment.CurrentDirectory, ".env"))
        Dim directory As DirectoryInfo = New DirectoryInfo(AppDomain.CurrentDomain.BaseDirectory)
    ...
    Private Shared Sub LoadEnvironmentFile(path As String, values As IDictionary(Of String, String))
        If String.IsNullOrWhiteSpace(path) OrElse Not File.Exists(path) Then Return
        For Each rawLine In File.ReadAllLines(path, Encoding.UTF8)
    ...
    Private Shared Sub OverrideFromProcessEnvironment(values As IDictionary(Of String, String), key As String)
        Dim value As String = Environment.GetEnvironmentVariable(key)
        If value IsNot Nothing Then values(key) = value
    End Sub

    Private Shared Function GetValue(values As IDictionary(Of String, String), key As String, fallback As String) As String
...
Public NotInheritable Class StockPoolRepository
        ...
        Try
            ...
            For index As Integer = 0 To request.Names.Count - 1
                ...
                If name.Length = 0 Then
                    rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = name, .Reason = "blank_name"})
                ElseIf Not seenNames.Add(name) Then
                    rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = name, .Reason = "duplicate_input"})
                Else
                    uniqueNames.Add(name)
                End If
            Next

            Dim byName As New Dictionary(Of String, List(Of StockPoolResolvedSymbol))(StringComparer.Ordinal)
            If uniqueNames.Count > 0 Then
                Using connection As New MySqlConnection(settings.BuildConnectionString())
                    ...
                    Using command As MySqlCommand = connection.CreateCommand()
                        ...
                        For index As Integer = 0 To uniqueNames.Count - 1
                            Dim parameterName As String = "@name" & index.ToString(CultureInfo.InvariantCulture)
                            placeholders.Add(parameterName)
                            command.Parameters.AddWithValue(parameterName, uniqueNames(index))
                        Next
                        command.CommandText =
                            "SELECT code, name, COALESCE(market, '') " &
                            "FROM g3_symbol_master " &
                            "WHERE delisted = 0 AND BINARY name IN (" & String.Join(",", placeholders) & ") " &
                            "ORDER BY name, code"
                        ...
                        Using reader As MySqlDataReader = command.ExecuteReader()
                            While reader.Read()
                                Dim item As New StockPoolResolvedSymbol With {
                                    ...
                                    .Market = If(reader.IsDBNull(2), String.Empty, reader.GetString(2).Trim())
                                }
                                Dim matches As List(Of StockPoolResolvedSymbol) = Nothing
                                If Not byName.TryGetValue(item.Name, matches) Then
                                    matches = New List(Of StockPoolResolvedSymbol)()
                                    byName(item.Name) = matches
                                End If
                                matches.Add(item)
                            End While
                        End Using
            ...
            For index As Integer = 0 To request.Names.Count - 1
                ...
                If request.Names.Take(index).Any(Function(previous) String.Equals(If(previous, String.Empty).Trim(), name, StringComparison.Ordinal)) Then Continue For

                Dim matches As List(Of StockPoolResolvedSymbol) = Nothing
                If Not byName.TryGetValue(name, matches) OrElse matches.Count = 0 Then
                    rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = name, .Reason = "exact_name_not_found"})
                ElseIf matches.Count <> 1 Then
                    rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = name, .Reason = "exact_name_ambiguous"})
                Else
    ...
    Public Function CreateCohort(request As StockPoolCohortCreateRequest) As ApiResponse
        ...
        If Not TryParseTradingDate(request.TradingDate, tradingDate) Then
            Return ApiResponse.Err("trading_date 형식은 yyyy-MM-dd 또는 yyyyMMdd여야 합니다.", 400)
        End If
        Dim captureTime As TimeSpan
        If Not TryParseCaptureTime(request.CaptureTime, captureTime) Then
            Return ApiResponse.Err("capture_time 형식은 HH:mm 또는 HHmm이어야 합니다.", 400)
        End If
        ...
        Try
            Dim settings As StockPoolDbSettings = StockPoolDbSettings.Load()
            Dim rejected As New List(Of StockPoolRejectedSymbol)()
            Dim validated As List(Of StockPoolCohortMemberRequest) = ValidateCohortMembers(settings, request.Members, rejected)
            If validated.Count = 0 Then
                Return ApiResponse.Err("저장 가능한 종목이 없습니다.", 400, New With {.rejected = rejected})
            End If
            ...
            Dim rawImportHash As String = ComputeImportHash(sourceType, conditionName, tradingDate, captureTime, request.TimeframeMinutes, validated)
            ...
            Using connection As New MySqlConnection(settings.BuildConnectionString())
                ...
                Using transaction As MySqlTransaction = connection.BeginTransaction()
                    Dim cohortId As Long
                    Using command As MySqlCommand = connection.CreateCommand()
                        command.Transaction = transaction
                        command.CommandText =
                            "INSERT INTO stock_pool_cohort " &
                            "(source_type, condition_name, trading_date, capture_time, timeframe_minutes, raw_import_hash) " &
                            "VALUES (@source_type, @condition_name, @trading_date, @capture_time, @timeframe_minutes, @raw_import_hash) " &
                        ...
                        command.Parameters.AddWithValue("@timeframe_minutes", request.TimeframeMinutes)
                        command.Parameters.AddWithValue("@raw_import_hash", rawImportHash)
                        command.ExecuteNonQuery()
                        cohortId = command.LastInsertedId
                    End Using

                    Using deleteCommand As MySqlCommand = connection.CreateCommand()
                        deleteCommand.Transaction = transaction
                        deleteCommand.CommandText = "DELETE FROM stock_pool_cohort_member WHERE cohort_id = @cohort_id"
                        deleteCommand.Parameters.AddWithValue("@cohort_id", cohortId)
                        deleteCommand.ExecuteNonQuery()
                    End Using

                    For index As Integer = 0 To validated.Count - 1
                        InsertCohortMember(connection, transaction, cohortId, index, validated(index))
                    Next
                    transaction.Commit()
    ...
    Private Shared Function ValidateCohortMembers(settings As StockPoolDbSettings, members As IList(Of StockPoolCohortMemberRequest), rejected As IList(Of StockPoolRejectedSymbol)) As List(Of StockPoolCohortMemberRequest)
        Dim codes As New List(Of String)()
        Dim seenCodes As New HashSet(Of String)(StringComparer.Ordinal)
        For index As Integer = 0 To members.Count - 1
            Dim code As String = If(members(index).Code, String.Empty).Trim()
            If code.Length = 0 Then
                rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = If(members(index).Name, String.Empty), .Reason = "blank_code"})
            ElseIf Not seenCodes.Add(code) Then
                rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = If(members(index).Name, String.Empty), .Reason = "duplicate_code"})
            Else
                codes.Add(code)
            End If
        Next

        Dim masterByCode As New Dictionary(Of String, StockPoolResolvedSymbol)(StringComparer.Ordinal)
            ...
            Using connection As New MySqlConnection(settings.BuildConnectionString())
                ...
                Using command As MySqlCommand = connection.CreateCommand()
                    ...
                    For index As Integer = 0 To codes.Count - 1
                        Dim parameterName As String = "@code" & index.ToString(CultureInfo.InvariantCulture)
                        placeholders.Add(parameterName)
                        command.Parameters.AddWithValue(parameterName, codes(index))
                    Next
                    command.CommandText =
                        "SELECT code, name, COALESCE(market, '') FROM g3_symbol_master " &
                        "WHERE delisted = 0 AND code IN (" & String.Join(",", placeholders) & ")"
                    Using reader As MySqlDataReader = command.ExecuteReader()
                        While reader.Read()
                            Dim item As New StockPoolResolvedSymbol With {
                                .Code = reader.GetString(0).Trim(),
        ...
        Dim acceptedCodes As New HashSet(Of String)(StringComparer.Ordinal)
        For index As Integer = 0 To members.Count - 1
            Dim input As StockPoolCohortMemberRequest = members(index)
            Dim code As String = If(input.Code, String.Empty).Trim()
            If code.Length = 0 OrElse acceptedCodes.Contains(code) Then Continue For
            Dim master As StockPoolResolvedSymbol = Nothing
            If Not masterByCode.TryGetValue(code, master) Then
                rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = If(input.Name, String.Empty), .Reason = "code_not_found"})
                Continue For
            End If
            ...
            If Not String.Equals(inputName, master.Name, StringComparison.Ordinal) Then
                rejected.Add(New StockPoolRejectedSymbol With {.InputIndex = index, .Name = inputName, .Reason = "code_name_mismatch"})
                Continue For
            End If
    ...
    Private Shared Sub EnsureSchema(connection As MySqlConnection)
        Dim statements As String() = {
            ...
            "UNIQUE KEY uq_stock_pool_cohort_import (source_type, condition_name, trading_date, capture_time, raw_import_hash)," &
            "KEY ix_stock_pool_cohort_lookup (trading_date, condition_name, capture_time)" &
            ...
            "UNIQUE KEY uq_stock_pool_cohort_member_ordinal (cohort_id, member_ordinal)," &
            "CONSTRAINT fk_stock_pool_cohort_member_cohort FOREIGN KEY (cohort_id) REFERENCES stock_pool_cohort(cohort_id) ON DELETE CASCADE" &
            ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci"
        }
    ...
    Private Shared Sub InsertCohortMember(connection As MySqlConnection, transaction As MySqlTransaction, cohortId As Long, ordinal As Integer, member As StockPoolCohortMemberRequest)
        Using command As MySqlCommand = connection.CreateCommand()
            command.Transaction = transaction
            command.CommandText =
                "INSERT INTO stock_pool_cohort_member " &
                "(cohort_id, member_ordinal, code, name, market, return_1m_label, return_3m_label, return_7h_label, maximum_return_label, capture_volume, other_label) " &
                "VALUES (@cohort_id, @member_ordinal, @code, @name, @market, @return_1m, @return_3m, @return_7h, @maximum_return, @capture_volume, @other_label)"
            ...
            command.Parameters.AddWithValue("@market", If(String.IsNullOrWhiteSpace(member.Market), CType(DBNull.Value, Object), member.Market))
            ...
            command.Parameters.AddWithValue("@maximum_return", NullableDbValue(member.MaximumReturnLabel))
            command.Parameters.AddWithValue("@capture_volume", NullableDbValue(member.CaptureVolume))
            command.Parameters.AddWithValue("@other_label", NullableDbValue(member.OtherLabel))
            command.ExecuteNonQuery()
        End Using
    End Sub

    Private Shared Function NullableDbValue(Of T As Structure)(value As Nullable(Of T)) As Object
        If value.HasValue Then Return value.Value
        Return DBNull.Value
    End Function

    Private Shared Function TryParseTradingDate(value As String, ByRef result As DateTime) As Boolean
        Return DateTime.TryParseExact(If(value, String.Empty).Trim(), {"yyyy-MM-dd", "yyyyMMdd"}, CultureInfo.InvariantCulture, DateTimeStyles.None, result)
    End Function

    Private Shared Function TryParseCaptureTime(value As String, ByRef result As TimeSpan) As Boolean
        Dim parsed As DateTime
        If DateTime.TryParseExact(If(value, String.Empty).Trim(), {"HH:mm", "HHmm", "HH:mm:ss"}, CultureInfo.InvariantCulture, DateTimeStyles.None, parsed) Then
    ...
    Private Shared Function ComputeImportHash(sourceType As String, conditionName As String, tradingDate As DateTime, captureTime As TimeSpan, timeframeMinutes As Integer, members As IEnumerable(Of StockPoolCohortMemberRequest)) As String
        Dim builder As New StringBuilder()
        builder.Append(sourceType).Append("|").Append(conditionName).Append("|")
        builder.Append(tradingDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)).Append("|")
        builder.Append(captureTime.ToString()).Append("|").Append(timeframeMinutes.ToString(CultureInfo.InvariantCulture)).AppendLine()
        For Each member In members
            builder.Append(member.Code).Append("|").Append(member.Name).Append("|").Append(member.Market).Append("|")
            ...
            If member.CaptureVolume.HasValue Then builder.Append(member.CaptureVolume.Value.ToString(CultureInfo.InvariantCulture))
            builder.Append("|")
    ...
    Private Shared Sub AppendNullable(builder As StringBuilder, value As Nullable(Of Decimal))
        If value.HasValue Then builder.Append(value.Value.ToString("0.######", CultureInfo.InvariantCulture))
        builder.Append("|")
    End Sub
End Class
